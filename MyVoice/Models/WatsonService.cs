...
namespace MyVoice.Models
{
    public static class WatsonService
    {
        ...
        public static Task<string> GetVoices()
        {
            ...
            {
                var response = client.GetAsync(URL + "voices");
        ...
        public static bool GetTTS(Document phrase)
        {
            ...
            {
                using (var handler = new HttpClientHandler { Credentials = new NetworkCredential(USERNAME, PASSWORD) })
                using (var client = new HttpClient(handler)) using (var response = client.PostAsJsonAsync(URL + "synthesize?voice=en-US_AllisonVoice", new TTSRequest() { text = phrase["Text"] }))
